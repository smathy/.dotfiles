...
gb() {
  _old_git_exists=$git_exists
  git_exists=`/opt/local/bin/git branch 2>/dev/null | sed -ne'/^\* /s///p'`

  if [[ "$_old_git_exists" != "$git_exists" ]]; then
    old_git_exists=$_old_git_exists
    unset _old_git_exists
  fi
...
current_git_stash() {
  git_stash=`/opt/local/bin/git stash list 2>/dev/null | /usr/bin/wc -l | sed 's/ *//g'`
...
reset_db() {
  ...
  for db in $base_db ${base_db}_test; do
    psql -U $user -d $root_db -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '$db' AND pid <> pg_backend_pid()"
    psql -U $user -d $root_db -c "DROP DATABASE $db"
    psql -U $user -d $root_db -c "CREATE DATABASE $db"
    pg_restore -d $db -U $user $bup
    psql -U $user -d $db -c "UPDATE users SET encrypted_address_1 = NULL, encrypted_address_2 = NULL, encrypted_city = NULL, encrypted_state = NULL, encrypted_zip = NULL, encrypted_country = NULL, encrypted_phone = NULL, encrypted_date_of_birth = NULL, encrypted_ssn = NULL"
    psql -U $user -d $db -c "UPDATE companies SET encrypted_tax_id = NULL, encrypted_address_1 = NULL, encrypted_address_2 = NULL, encrypted_city = NULL, encrypted_state = NULL, encrypted_zip = NULL, encrypted_country = NULL, encrypted_phone = NULL"
  done
}
